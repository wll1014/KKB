<template>
  <!--TODO: 此页面以取消后续删除 -- 20200318-->
    <div>
      <div class="tem-develop-block">
        <div>后续移除此页面，可忽略</div>
      </div>
...
<script>
  export default {
    ...
    methods:{
      //获取目前选择的数据
      getSelectedData(){
        return this.$refs.theOD.getSelectedData()
      },
      ...
      async loadNode(node,isNew=false){
        ...
        if(data[this.treeProps.children] && data[this.treeProps.children].length > 0){
          ...
          // 懒加载数据
          let roomInfo=await this.getDeviceInfoFromMachineRoom(data.moid)
          // console.log(userInfo)
          // this.allRoomInfo.forEach((item)=>{
          //   if(item.moid===data.moid){
          //     this.$set(item,'info',roomInfo)
          //   }
          // })
          return roomInfo;
        }
      ...
      async treeFilterSearch(val){
        if(val){
          ...
          let roomInfo = await this.$api.homePage.getDeviceInfoBySearch(params)
          ...
          this.allRoomInfo=await this.$api.homePage.getAllMachineRoomInfo()
        }
      },
          ...
          for(let item of v){
            if(item.hasOwnProperty(this.treeProps.children)){
              ...
              if(filter){ //如果有过滤条件
                ...
                roomInfo=await this.getDeviceInfoFromMachineRoom(item.moid)
              }
      ...
      // 获取所有机房
      async getAllMachineRoomInfo(){
        this.allRoomInfo=await this.$api.homePage.getAllMachineRoomInfo()
      },

      //获取外设服务器
      async getDeviceInfoFromMachineRoom(moid){
        let start=0
        let count=50
        ...
        while (temCacheLength>=count){
          ...
          let data=await this.$api.homePage.getDeviceInfoFromMachineRoom(moid,params)
