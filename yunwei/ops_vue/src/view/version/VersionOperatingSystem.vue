...
<script>
  export default {
    ...
    methods: {
			ftpDownUsingClose(){
				this.$api.version.upOpClose().then(res=>{
      ...
      getData(params){
        ...
        // this.versionTable = [{"version":1,"uploader":"zhangsan","id":6}]
        this.$api.version.getOpData(params).then(res=>{
          if(res.data.success===1){
      ...
      upVersion(){                                        //上传
				...
				this.upVersionUrl = ''
				this.$api.version.getOpVersionUrl().then(res=>{
					if(res.data.success === 1){
      ...
      //       this.$message(res.data.msg)
      //     }
      //   }).catch(err=>{this.sendRequest = false})
      },
			delVersion(){                                   //点击删除键
			...
			deleteRowAll(){                 //确认删除
			  ...
			  this.$api.version.versionOpDelete(param).then(res =>{
      ...
      EditVersionDialogSure(){          //编辑确定版本信息
        var params=this.EditVersion
				this.start=0
        this.$api.version.editOpVersion(params).then(res=>{
          if(res.data.success===1){
      ...
      downVersion(index,versionTable){                  //下载请求url
				...
				this.$api.version.downOpUrl(this.versionTable[this.index].id).then(res=>{
				  if(res.data.success===1){
						this.sendRequest = true
						this.ftpUsing = false	
						// for(var i in res.data.msg){
