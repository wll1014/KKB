...
export default {
  ...
  methods: {
		ftpDownUsingClose(index,versionTable){
			this.$api.version.upClose().then(res=>{
    ...
    getData(params){
      ...
      this.$api.version.getData(params).then(res=>{
        if(res.data.success===1){
    ...
    upVersion(){                                        //上传
			...
			this.upVersionUrl = ''
      this.$api.version.getVersionUrl().then(res=>{
        if(res.data.success === 1){
    ...
    deleteRowAll(){                 //确认删除
      ...
      this.$api.version.versionDelete(param).then(res =>{
    ...
    EditVersionDialogSure(){          //编辑确定版本信息
      var params=this.EditVersion
			// console.log(params)
			this.start = 0
      this.$api.version.editVersion(params).then(res=>{
        if(res.data.success===1){
					this.start=0
    ...
    downVersion(index,versionTable){                  //下载请求url
      this.downVersionDialog = true
			this.downVersionUrl = ''
			this.index = index
      this.$api.version.downUrl(this.versionTable[this.index].id).then(res=>{
        if(res.data.success===1){
					this.sendRequest = true
					this.ftpUsing=false
...
.povPost.el-popover .popper__arrow{
  display: none;
}
</style>
