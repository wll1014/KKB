...
<script>
export default {
  name: "TheOrganizationMachineRoom",
  components:{
  ...
  methods:{
    //获取目前选择的数据
    getSelectedData(){
      return this.$refs.theOMR.getSelectedData()
    },
    ...
    async loadNode(node,isNew=false){
      ...
      if(data[this.treeProps.children] && data[this.treeProps.children].length > 0){
        ...
        // 懒加载数据
        let roomInfo=await this.getMachineRoomInfo(data.moid)
        return roomInfo;
      }
    },
    ...
    async treeFilterSearch(val){
      if(val){
        ...
        let roomInfo = await this.$api.homePage.getMachineRoomInfoBySearch(params)
        ...
        for(let item of v){
          if(item.hasOwnProperty(this.treeProps.children)){
            ...
            if(filter){ //如果有过滤条件
              ...
              roomInfo=await this.getMachineRoomInfo(item.moid)
            }
    ...
    async getMachineRoomInfo(moid){
      ...
      while (temCacheLength>=count){
        ...
        let data=await this.$api.homePage.getMachineRoomInfo(moid,params)
