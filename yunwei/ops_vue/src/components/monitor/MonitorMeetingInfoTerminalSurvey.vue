<template>
    <div>
      <div style="height: 580px;width: 100%;position: relative;" >
        <el-checkbox v-model="checkboxLegendSelected" @change="toggleLegend" class="checkbox-terminal-confsummary">全部</el-checkbox>
        <ChartExtendLargeModel ChartID="IDMeetingInfoTerminalConfSummary"
                               ...
                               @plugin-event="chartPluginChange"></ChartExtendLargeModel>
      </div>
    </div>
</template>
  ...
  methods:{
    ...
    chartPluginChange(act){
      ...
      if(act === 'plus' || act === 'minus'){
        let interval= this.extraPlugin.dataInterval.interval
        interval = act === 'plus' ? interval + 10 : act === 'minus' ? interval - 10 : interval
        interval = interval <= 10 ? 10 :interval >= 200 ? 200 : interval
        let newConf = {
          interval:interval,
          suffix:'秒'
        }
